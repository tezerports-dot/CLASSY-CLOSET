...
void main() {
  ...
  test('initialize creates the schema and seeds first-run data', () async {
    ...
    expect(store.storeProfile, isNull, reason: 'a fresh install has no store profile yet');
  });

  test('the seeded admin can sign in and a wrong password is rejected', () async {
    await store.initialize();
  ...
  test('saving a store profile persists the receipt prefix across a reload', () async {
    await store.initialize();
    await store.saveStoreProfile(
      const StoreProfile(storeName: 'Classy Closet', currencySymbol: r'$', receiptNumberPrefix: 'CC'),
    );
  ...
  test('a split-payment checkout records the sale, splits cash/card and draws down stock', () async {
    await store.initialize();
    ...
    final sale = await store.checkout(paid: 20, paymentMethod: 'split', cashAmount: 12, cardAmount: 8);
    ...
    expect(store.products.single.stock, 8, reason: 'two units were sold from a stock of ten');
    final movements = await db.select(db.inventoryMovements).get();
    expect(movements.where((m) => m.movementType == 'sale').single.quantity, -2);
  });
  ...
  test('an unpaid balance is carried onto the customer ledger', () async {
    ...
    final customer = store.customers.firstWhere((c) => c.name == 'Walk-in Customer');

    store.addToCart(store.products.single);
    await store.checkout(customer: customer, paid: 10, paymentMethod: 'cash', cashAmount: 10);
