...
class StoreProfileForm extends StatefulWidget {
  const StoreProfileForm({required this.store, required this.onSaved, super.key});
...
class _StoreProfileFormState extends State<StoreProfileForm> {
  ...
  Widget build(BuildContext context) {
    final logo = _logoPath == null || !File(_logoPath!).existsSync() ? null : File(_logoPath!);
    return Form(
      key: _formKey,
      child: Column(
        ...
        children: [
          if (logo != null) ...[
            Align(alignment: Alignment.centerLeft, child: Image.file(logo, width: 96, height: 96, fit: BoxFit.contain)),
          ...
          FilledButton.icon(onPressed: _save, icon: const Icon(Icons.save), label: const Text('Save store profile')),
        ],
      ),
    );
  }
